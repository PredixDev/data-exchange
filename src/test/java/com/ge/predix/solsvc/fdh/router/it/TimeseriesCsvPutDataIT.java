...
{
        FdhRouterApplication.class, AssetPutDataHandlerImpl.class, TimeseriesPutDataHandler.class
})
@WebAppConfiguration
@IntegrationTest(
{
...
{
    private static final Logger logger    = LoggerFactory.getLogger(TimeseriesCsvPutDataIT.class);
            ...
            throws Exception
    {
        ...
        this.base = new URL("http://localhost:" + this.localServerPort); // $NON-NLS-2$
    }
            ...
            throws Exception
    {
        ...
        PutFieldDataResult response = this.template.postForObject(
                this.base + "/services/fdhrouter/fielddatahandler/putfielddatafile", httpEntity,
                PutFieldDataResult.class);
    ...
    private PutFieldDataRequest createNewPutRequest()
    {
        ...
        fieldIdentifier.setSource("PREDIX_ASSET"); //$NON-NLS-1$
        field.setFieldIdentifier(fieldIdentifier);
        MetaData metaData = new MetaData();
        metaData.setSource("PREDIX_ASSET"); //$NON-NLS-1$
        metaData.setDescription(" Meta Data for Time series file"); //$NON-NLS-1$
        metaData.setName("TimeSeries CSV Data File request"); //$NON-NLS-1$
        ...
        fieldIdentifier.setSource("PREDIX_TIMESERIES"); //$NON-NLS-1$
        field.setFieldIdentifier(fieldIdentifier);
        DataFile dataFile = new DataFile();
        dataFile.setName("Time Series Test File"); //$NON-NLS-1$
        fieldData.getField().add(field);
